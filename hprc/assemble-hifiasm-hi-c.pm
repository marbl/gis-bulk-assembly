...
sub createHifiasmHiC ($$$$$$$$$$$$) {
  ...
  if (!$missi && !$compl && !$unava && !-e "$sdir/$flav.sh") {
    ...
    print CMD "  $roft/hifiasm/hifiasm -o $flav/assembly \\\n";
    ...
    print CMD "    $params      \\\n";
    print CMD "  && \\\n";
    print CMD "  cat $flav/assembly.hic.hap1.p_ctg.gfa     |awk '{if (match(\$1, \"^S\")) { print \">\"\$2; print \$3}}' | fold -c > $flav/assembly.haplotype1.fasta && \\\n";
    print CMD "  cat $flav/assembly.hic.hap2.p_ctg.gfa     |awk '{if (match(\$1, \"^S\")) { print \">\"\$2; print \$3}}' | fold -c > $flav/assembly.haplotype2.fasta && \\\n";
    print CMD "  cat $flav/assembly.hic.hap[12].p_ctg.gfa  |awk '{if (match(\$1, \"^S\")) { print \">\"\$2; print \$3}}' | fold -c > $flav/assembly.fasta && \\\n";
    print CMD "  cat $flav/assembly.hic.hap[12].p_ctg.gfa  |awk '{if (match(\$1, \"^S\")) { print \">utig4-\"\$2; print \$3}}' | seqtk hpc - | fold -c > $flav/contigs.fasta && \\\n";
    print CMD "  cat $flav/assembly.hic.hap[12].p_ctg.gfa  |awk '{if (match(\$1, \"^S\")) { print \"path \"\$2\" utig4-\"\$2 } }' > $flav/assembly.scfmap && \\\n";
    print CMD "  cat $flav/assembly.hic.hap[12].p_ctg.gfa  |awk '{if (match(\$1, \"^S\")) { print \$1\"\\tutig4-\"\$2\"\\t*\\tLN:i:\"length(\$3); } else if (match(\$1, \"^L\")) print \$1\"\\tutig4-\"\$2\"\\t\"\$3\"\\tutig4-\"\$4\"\\t\"\$5\"\\t\"\$6 }' > $flav/assembly.homopolymer-compressed.noseq.gfa && \\\n";
    print CMD "  cat $flav/assembly.hic.hap[12].p_ctg.gfa  |awk '{if (match(\$1, \"^S\")) { gsub(/A+/, \"A\", \$3); gsub(/C+/, \"C\", \$3); gsub(/G+/, \"G\", \$3); gsub(/T+/, \"T\", \$3);  print \$1\"\\tutig4-\"\$2\"\\t\"\$3\"\\tLN:i:\"length(\$3); } else if (match(\$1, \"^L\")) print \$1\"\\tutig4-\"\$2\"\\t\"\$3\"\\tutig4-\"\$4\"\\t\"\$5\"\\t\"\$6 }' > $flav/assembly.homopolymer-compressed.gfa && \\\n";
